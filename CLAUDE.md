...
**Legacy redirects on the Vercel adapter** <!-- added: 2026-05-23, corrected: 2026-05-23 -->: `redirects` in `astro.config.mjs` become **exact-anchored** regex routes in `.vercel/output/config.json` (`^/path$`), with NO automatic trailing-slash normalization. ⚠️ **Listing both the `/foo` and `/foo/` keys does NOT work** (this was the prior belief and it was wrong — caught by the pre-deploy review): `@astrojs/vercel` builds each redirect's `src` from the path *segments* (`getRedirects` → `getMatchPattern`), which drop the trailing slash, so both keys collapse to the identical `^/foo$` and are emitted twice as exact duplicates. With Astro's default `trailingSlash: 'ignore'`, `getTransformedRoutes` gets `trailingSlash: undefined` so no normalization route is emitted and the regex has no optional slash — the canonical trailing-slash form `/foo/` (the directory-format canonical search engines index) matches no route and 404s on the real Vercel edge. `astro dev` matches both forms, masking it. The emitted regex can't be shaped through the `redirects` config, so the three legacy `/blog/<slug>` 301s are made slash-tolerant by the **`patchLegacyBlogRedirects` Astro integration** (inline in `astro.config.mjs`): it runs in `astro:build:done` *after* the adapter writes config.json (verified by build-log ordering), rewrites the trailing `$` → `/?$` on the legacy blog 301s, dedupes, and throws (no silent no-op) if it finds nothing to patch. Running inside `astro build` means it applies whether Vercel runs `astro build` or `npm run build`. Verify against the built `.vercel/output/config.json` + the Vercel routing model, never `astro dev`.

**ContactCTA locale defaults** <!-- added: 2026-05-23 -->: `ContactCTA.astro` defaults **all** form-chrome strings to English (`emailLabel`, `messageLabel`, `placeholder`, `fallbackBefore`, `successMessage`, `sendingText`, `errorText`, and the Web3Forms `subject`). German pages MUST pass the German overrides or the form renders half-English (`Email`, `What are you working on?`, `Or email me directly at`) amid German copy — easy to miss because the form looks fine in isolation. Copy the full prop set from `/de/kontakt` or `/de/arbeiten` when adding a contact form to any new DE page. (This is the same trap as `BlogCTA.astro`'s `lang` prop, but ContactCTA has no single `lang` switch — each string is its own prop.) A half-English form on `/de/` + `/de/leistungen` shipped past the first build and was caught only by the pre-deploy review (M2).
...
**Bare `{...}` in MDX prose parses as a JSX expression** <!-- added: 2026-07-08 -->: MDX evaluates `{...}` in body text as JavaScript, so a placeholder like `{task}` in prose fails `astro build` with `ReferenceError: task is not defined` — a render-time error, so it slips past a quick source eyeball; always run `npm run build`. Wrap placeholders in an inline code span (`` `{task}` ``) or escape the brace (`\{`). Fenced code blocks and `mermaid` blocks are exempt (braces there are literal), which is why the diagram/code-heavy posts never hit it.
