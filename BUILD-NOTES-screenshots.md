...
## Re-skin of the editorial-CMS screenshots (2026-05-23, review B1 + M1)

The independent pre-deploy review (`REVIEW-FINDINGS.md`) caught two leaks that a
text strip-grep of `dist/` cannot see, because they are text *rendered inside the
PNGs*:

- **B1 (blocker):** the CMS header band read "Redaktionelles Workflow-CMS für die
  **Angebotslandkarte 2.0**" in `editorial-personas.png`,
  `editorial-intake-queue.png`, `editorial-reminders.png`. "Angebotslandkarte" is
  a strip-list term (the buyer's product name); the capture-time DOM neutralizer
  stripped `KNE`/`Kompetenznetz`/`Einsamkeit` but **not** "Angebotslandkarte".
- **M1 (major):** the synthetic seed painted an unmistakable
  bereavement / elderly-isolation domain (Telefonseelsorge, Trauergruppen,
  Männer/Frauen 60+, Seniorentreff, Trauerbegleitung nach Suizid …). VISION
  states the loneliness topic itself is identifying.

**Fix:** all six `editorial-*.png` were re-captured against a **domain-neutral
re-skin** of the demo, run locally. The new seed is a generic municipal /
community-services directory (Bürgertelefon, Verbraucher-/Familien-/
Schuldnerberatung, Online-Kurse, Stadtteilzentren, Stadtbüchereien, VHS-Sprachkurs,
Lesekreise) across the same German geography, preserving the identical schema,
ids, status distribution (14 veröffentlicht / 2 eingereicht / 2 änderungswunsch /
2 entwurf), relationships, audit timeline, and reminder cascade, so every
capability shot (persona switch, intake queue, before/after diff, audit log,
reminder escalation + preview outbox) renders exactly as before with neutral
content. The CMS header was relabelled to "Redaktions-CMS / Redaktionelles
Workflow-CMS · Demonstrator".

**How (and the demo-repo invariant):** the re-skin was applied as **temporary,
uncommitted local edits** to `~/projects/kne-cms-demo` (seed.sql, the bundled
`index.html` header + two domain-example strings, the SPA persona mirror
`src/lib/personas.js`, the actor-email constants in `src/views/redaktion.js` /
`functions/_lib/{writes,reminders}.js` / `functions/api/confirm/[token].js`, and
the reminder email templates in `functions/_lib/reminders.js` which had baked-in
buyer strings incl. the `landkarte@kompetenznetz-einsamkeit.de` mailbox and an
"ISS e.V. Frankfurt" footer). The demo was rebuilt (`npm run build` regenerates
`functions/_lib/seed-data.js`, which the `/api/demo/reset` button the capture
script clicks re-runs) + `npm run db:reset` + `wrangler pages dev`, then captured.
**The kne-cms-demo repo was restored to a fully clean state afterwards
(`git checkout`); nothing there was committed, pushed, or redeployed** — its
KNE branding + loneliness seed are intentional (the live demo is referenced in a
submitted bid). Only this repo's `public/work/editorial-*.png` changed.

**Each new PNG was visually verified by reading the image**: zero
"Angebotslandkarte", zero loneliness/bereavement/Senioren-isolation content, zero
`KNE`/`ISS`/`Kompetenznetz`/`Einsamkeit`, zero URL bar, zero logo. The
`offer-map-*.png` were already clean and were left untouched.

**Capture-script hardening (committed):** `scripts/capture-work-screenshots.mjs`
now also strips "Angebotslandkarte" (→ "Datenbestand", and "Angebotslandkarte 2.0"
→ "Demonstrator") in `CMS_NEUTRALIZE` and flags it in `CMS_STRIPCHECK`, so a future
re-capture cannot reintroduce the B1 header leak even without the source relabel.
The neutralizer deliberately does **not** touch the site's approved neutral term
"Angebotskarte".
